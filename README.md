...
### Option A — Bulk import all 16 skills (recommended)

Upload [`dist/exchek-all-skills.zip`](dist/exchek-all-skills.zip) via the Manus skills upload modal:

1. In Manus, open the skills upload modal.
2. Drop in `dist/exchek-all-skills.zip`.
3. Manus reads the zip and registers each `<skill>/SKILL.md` it finds.

### Option B — Install one skill at a time

Each skill in `dist/individual/` is a standalone `.skill` file (which is a zip with `<skill-name>/SKILL.md` at root). Upload one to register that skill only — useful if you only want, say, ECCN classification but not the full compliance program toolkit.

```
dist/individual/exchek-classify.skill
dist/individual/exchek-csl.skill
... (16 total)
```

### File format that Manus expects
...
Every skill in this repo conforms: each `<skill-name>/` directory contains a `SKILL.md` with `name:` and `description:` in YAML frontmatter, plus a `references/` directory with regulatory guidance the skill body links to.
...
└── dist/                 # Build output (generated by build.sh)
    ├── exchek-all-skills.zip   # Bulk import — all 16 skills in one zip
    └── individual/             # One .skill file per skill
...
The script:
1. Zips each `skills/<skill-name>/` directory into `dist/individual/<skill-name>.skill`.
2. Zips all 16 skill directories into `dist/exchek-all-skills.zip` for bulk import.

Re-run after editing any `SKILL.md` or reference file.
